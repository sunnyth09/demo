<template>
  <div class="container mx-auto px-4 py-8 bg-gray-50 min-h-screen">
    <div class="flex flex-col md:flex-row gap-6">
      ...
      <div class="w-full md:w-1/4 lg:w-1/5">
        <div class="bg-white rounded-lg shadow-sm border overflow-hidden">
          <div class="px-4 py-3 border-b-2 border-primary/50 bg-white">
            <h2 class="font-bold text-gray-800 uppercase flex items-center gap-2">
              ...
              <div v-for="cat1 in categoryTree" :key="cat1.id" class="space-y-1">
                <div class="flex items-center group">
                  ...
                  <button 
                    ...
                    :class="[
                      'flex-1 text-left py-2 pr-2 text-sm transition-colors',
                ...
                <div 
                  ...
                  class="ml-6 border-l border-gray-100 pl-1 space-y-1"
                >
                  <div v-for="cat2 in cat1.children" :key="cat2.id">
                    <div class="flex items-center group">
                      ...
                      <button 
                        ...
                        :class="[
                          'flex-1 text-left py-1.5 pr-2 text-sm transition-colors',
                    ...
                    <div 
                      ...
                      class="ml-5 border-l border-gray-100 pl-1 space-y-1"
                    >
                      <div v-for="cat3 in cat2.children" :key="cat3.id">
                         <div class="flex items-center pl-6">
                            <button 
                              ...
                              :class="[
                                'flex-1 text-left py-1 pr-2 text-xs transition-colors',
      ...
      <div class="flex-1">
        <!-- Sort Bar -->
        <div class="bg-white p-4 rounded-xl shadow-sm border mb-6 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <span class="text-sm font-medium text-gray-500 whitespace-nowrap">Sắp xếp theo:</span>
        ...
        <div v-else-if="products.length > 0" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4">
          <div 
          ...
          >
            ...
            <div v-if="product.quantity === 0" class="absolute top-2 left-2 z-10">
               <span class="bg-gray-900/90 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">HẾT HÀNG</span>
            </div>
            <div v-else-if="calculateDiscount(product.price, product.original_price) > 0" class="absolute top-2 left-2 z-10">
               <span class="bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">
                 -{{ calculateDiscount(product.price, product.original_price) }}%
               </span>
            </div>

            <!-- Image -->
            <div class="relative aspect-[3/4] overflow-hidden p-0 bg-transparent">
              <img 
                :src="product.thumbnail || 'https://via.placeholder.com/300x400?text=No+Image'" 
                :alt="product.name"
                class="w-full h-full object-contain mix-blend-multiply transition-transform duration-500 scale-110 group-hover:scale-125"
              />
            </div>
            ...
            <div class="p-3 flex flex-col flex-1">
              <h3 
                class="text-base leading-tight font-medium text-gray-800 line-clamp-2 mb-1 group-hover:text-primary transition-colors cursor-pointer min-h-[44px]"
                :title="product.name"
              >
                {{ product.name }}
              </h3>
        ...
        <div v-else class="text-center py-20 bg-white rounded-lg border">
          ...
          <p class="mt-1 text-sm text-gray-500">Thử thay đổi bộ lọc hoặc tìm kiếm từ khóa khác.</p>
        </div>

        <!-- Pagination -->
        <div v-if="products.length > 0 && totalPages > 1" class="flex justify-center mt-8 gap-2">
           <button 
