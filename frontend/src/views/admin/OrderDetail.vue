<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        <h2 class="text-2xl font-bold">
          Chi tiết đơn hàng #{{ order?.order_code ? order.order_code.slice(0, 8).toUpperCase() : order?.id }}
        </h2>
    ...
    <div v-else-if="order" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-card rounded-xl border p-6">
          ...
          <p v-if="order.status === 'delivered'" class="text-sm text-green-600 mt-3">
            Đơn hàng đã hoàn thành, không thể thay đổi trạng thái.
          </p>
          <p v-if="order.status === 'cancelled'" class="text-sm text-red-600 mt-3">
            Đơn hàng đã bị hủy.
          </p>
        </div>
        ...
        <!-- Order Items -->
        <div class="bg-card rounded-xl border p-6">
          <h3 class="text-lg font-bold mb-4">Sản phẩm ({{ order.items?.length || 0 }})</h3>
          <div class="space-y-4">
            <div v-for="item in order.items" :key="item.id" class="flex items-center gap-4 p-3 border rounded-lg">
              <div class="w-16 h-16 bg-gray-100 rounded-md shrink-0 overflow-hidden">
              ...
              <div class="text-right">
                ...
                <p class="font-bold text-primary">{{ formatCurrency(item.total_price) }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-card rounded-xl border p-6">
          ...
          <div class="space-y-3 text-sm">
            ...
            <div v-if="order.note">
              ...
              <p class="font-medium text-yellow-700 bg-yellow-50 px-2 py-1 rounded mt-1">{{ order.note }}</p>
            </div>
          </div>
        </div>
    ...
    <div v-else class="text-center py-12">
      ...
      <router-link to="/admin/orders" class="text-primary hover:underline mt-2 inline-block">Quay lại danh sách</router-link>
    </div>
  </div>
</template>
...
const updateOrderStatus = async () => {
  ...
  updating.value = true
  try {
    const res = await fetch(`${API_URL}/orders/admin/${order.value.id}/status`, {
      method: 'PUT',
      headers: {
