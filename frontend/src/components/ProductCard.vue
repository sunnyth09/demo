<template>
  <div 
    class="group bg-card rounded-lg border border-border hover:border-primary/30 hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-300 flex flex-col overflow-hidden relative"
  >
    <!-- Badge -->
    <div v-if="product.quantity === 0" class="absolute top-2 left-2 z-10">
      <span class="bg-foreground text-background text-[10px] font-bold px-2 py-1 rounded shadow-sm">HẾT HÀNG</span>
    </div>
    <div v-else-if="calculateDiscount(product.price, product.original_price) > 0" class="absolute top-2 left-2 z-10">
      <span class="bg-destructive text-white text-[10px] font-bold px-2 py-1 rounded shadow-sm">
    ...
    >
      ...
      <div class="absolute inset-x-0 bottom-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 flex justify-center gap-2 bg-gradient-to-t from-black/50 to-transparent pt-8">
        ...
        <button 
          ...
          class="h-8 w-8 rounded-full bg-white flex items-center justify-center hover:bg-red-500 hover:text-white transition-colors shadow-lg"
    ...
    <div class="p-4 flex flex-col flex-1">
      <div class="mb-1 text-[10px] text-muted-foreground uppercase tracking-wider font-semibold" v-if="product.category_name">
      ...
      <div class="mt-auto">
        <div class="flex items-baseline gap-2">
          ...
          <span v-if="product.original_price && product.original_price > product.price" class="text-xs text-muted-foreground line-through">
        ...
        <div class="flex items-center gap-1 mt-1.5">
          ...
          <span class="text-[10px] text-muted-foreground">({{ product.review_count || 0 }})</span>
        </div>
      </div>
    </div>
  </div>
...
defineEmits(['click-product', 'add-to-cart', 'toggle-favorite'])
</script>
