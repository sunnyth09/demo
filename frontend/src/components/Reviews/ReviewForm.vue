  ...
  <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
    ...
    >
      ...
      <p class="text-gray-600 mb-6 max-w-md mx-auto">
        Vui lòng đăng nhập để chia sẻ đánh giá của bạn về sản phẩm này.
      </p>
      <router-link
        to="/login"
    ...
    <div
      ...
      class="text-center py-12 bg-green-50 rounded-2xl border border-green-100"
    >
      <div
    ...
    <div v-else>
      <form @submit.prevent="submitReview" novalidate class="space-y-6">
        <!-- User Info & Rating -->
        <div class="flex items-start gap-4">
          <!-- Avatar -->
          <div
            class="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-lg hidden md:flex shrink-0"
          >
          ...
          <div class="flex-1 space-y-4">
            ...
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-3"
                >Bạn cảm thấy sản phẩm thế nào?</label
              >
              <div class="flex items-center gap-4">
                ...
                <span
                  v-if="hoverRating || rating"
                  class="text-sm font-medium text-primary bg-primary/5 px-3 py-1 rounded-full animate-fade-in"
                >
...
const submitReview = async () => {
  ...
  if (!comment.value.trim()) {
    errors.comment = "Nội dung không được để trống";
    isValid = false;
  }
